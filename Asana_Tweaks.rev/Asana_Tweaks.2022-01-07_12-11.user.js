...
(function() {
		...
		function initAsanaEnhancer() {
			...
			let addToElement = document.querySelector(addButtonsToThisBar); //Get ONLY ONE element to add buttons to

			createHighlightButton(addToElement);
			create_btnToolBarToggle(addToElement);
			create_btnToolBarHide(addToElement);
				...
				if (toolBars){
					if (!toolBars.length && toolBars.style) {
						...
						for (let tlBarIdx=0; tlBarIdx<toolBars.length; tlBarIdx++){
							...
							indivToolBar.style.height = height;
						}
					}
				}
			}
		...
		function setAllToolBarsVis(visibility){
			let toolBarsToHideSelectors = [	//each of these selectors are classes WHICH COULD return MULTIPLE elements
	  ...
	  function unexpectedParameterValueError(messageTxt){
			console.log("unexpectedParameterValueError: " + messageTxt);
		}
    ...
    function nullError(caller, optText){
        ...
        if (optText) {
					if (LoggingOn) console.log (optText);
				}
        if (LoggingOn) console.log('---------------');
		...
		function create_btnToolBarToggle(addToElement){
			...
			if (LoggingOn) console.log('btnToolBarToggle click event set to conditionalHighlighting()');
    }
