...
(function() {
		...
		function initAsanaEnhancer() {
			...
			//let addButtonsToThisBarClassName = ".TopbarPageHeaderGlobalActions.AsanaPageTopbar-globalActions";
			let addButtonsToThisBarClassName = ".TopbarPageHeaderGlobalActions";
			let appendToElement = document.querySelector(addButtonsToThisBarClassName); //Get ONLY ONE element to add buttons to
			if (!appendToElement) {	// if this is NULL & therefore false, then we didn't find the `addButtonsToThisBarClassName`
				// this likely means that ASANA has changed the classname since we last ran this.
				//TopbarPageHeaderStructure ProjectPageHeader
				nullError(this, "missing '" + addButtonsToThisBarClassName + "' -- check HTML for this classname");
			}
